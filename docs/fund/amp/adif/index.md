...
Cualquier pareja de señales \(v_1\) y \(v_2\) puede descomponerse en dos componentes: una **componente diferencial** y una **componente común**.
...
Esta descomposición es muy útil porque permite estudiar por separado cómo responde el amplificador ante una diferencia entre entradas y cómo responde ante una señal común aplicada simultáneamente a ambas.
...
## 16. Conexión con el amplificador diferencial NMOS

La teoría anterior es general y sirve como base para estudiar distintas realizaciones prácticas de amplificadores diferenciales. En el caso de un amplificador diferencial construido con transistores NMOS, las dos entradas se aplican a las puertas de \(M_1\) y \(M_2\), mientras que las salidas se toman normalmente en sus drenadores.

La corriente total del par se fija mediante una fuente de corriente, que puede implementarse con un espejo de corriente. Esta corriente establece el punto de trabajo y determina, junto con las resistencias de carga y los parámetros de los transistores, las ganancias del circuito.

En ese caso, la ganancia diferencial dependerá principalmente de la transconductancia \(g_m\) de los transistores de entrada y de la resistencia equivalente de carga, mientras que la ganancia en modo común dependerá en gran medida de la calidad de la fuente de corriente de cola. Por tanto, el estudio general de \(v_d\), \(v_c\), \(A_d\), \(A_c\) y \(CMRR\) permite entender después el funcionamiento concreto del par diferencial NMOS.
